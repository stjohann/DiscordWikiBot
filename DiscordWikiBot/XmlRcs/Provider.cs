	...
	public class Provider
	{
		...
		private async Task Pinger_Exec()
		{
			try
			{
				...
				{
					this.ping();
					await Task.Delay(TimeSpan.FromSeconds(Configuration.PingWait), ct);
				}
			}
		...
		private async Task Provider_Exec()
		{
			...
			{
				var ct = close.Token;
				while (!this.streamReader.EndOfStream && this.IsConnected && !ct.IsCancellationRequested)
				{
		...
		private void kill()
		{
			...
			this.close = new CancellationTokenSource();
			close.Cancel();
		}
